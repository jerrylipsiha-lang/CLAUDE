...
## 📱 Приложение «Без цензуры» — диалог с Будько и Позовым

В папке [`phone-app/`](phone-app/) — мобильное приложение-болталка (PWA): переписка
сразу с двумя персонажами, Иваном Будько и Дмитрием Позовым, которые обсуждают всё
подряд без купюр. Работает офлайн со встроенным движком на 15 тем; опционально —
«живой режим» через Claude API. Ставится на телефон через «Добавить на главный экран».
Запуск: `cd phone-app && python3 -m http.server 8000`. Подробности — в
[`phone-app/README.md`](phone-app/README.md).
